...
import {MultiSigWallet, Tx, MultiSigWallet__EmptySigners, MultiSigWallet__AlreadyConfirmed, MultiSigWallet__MoreConfirmationsThanSigners, MultiSigWallet__ShouldBeAtLeastOneConfirmation, MultiSigWallet_SignersShouldBeUnique, MultiSigWallet__InsufficientBalance, MultiSigWallet__InvalidDeadline, MultiSigWallet__InsufficientTxConfirmations} from "../src/MultiSigWallet.sol";
...
contract MultiSigWalletTest is Test {
    ...
    function proposeValidTx(
        bool autoApprove
    ) internal returns (address proposer, uint256 nonce) {
        vm.prank(aliceAddr);
    ...
    function testMultiSigWalletConstructor() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MultiSigWallet_SignersShouldBeUnique.selector,
                0,
                1
            )
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MultiSigWallet__MoreConfirmationsThanSigners.selector,
                3,
                2
            )
        );
    ...
    function testIsUniqueAccounts() public view {
        ...
        (int n1Dif, int n2Dif) = wallet.isUniqueHarness(diffrentAccounts);

        assertTrue(
            n1Dif == -1 && n2Dif == -1,
            "Should be true when accounts are different"
        );

        (int n1, int n2) = wallet.isUniqueHarness(new address[](0));
        ...
        (int n1Repeated, int n2Repeated) = wallet.isUniqueHarness(
            repeatedAccounts
        );
    ...
    function testProposeTxWithIsufficientBalance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MultiSigWallet__InsufficientBalance.selector,
                txValue,
    ...
    function testProposeTxWithWrongDeadline() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(MultiSigWallet__InvalidDeadline.selector)
        );
        vm.prank(vm.addr(uint256(keccak256("alice"))));
        wallet.proposeTx(
            address(0x1),
    ...
    function testInsufficientTxConfirmations() external {
        (address proposer, uint256 nonce) = proposeValidTx(false);
        vm.startPrank(aliceAddr);
